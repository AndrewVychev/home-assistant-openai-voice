...
Теперь сателлит постоянно ждёт «Хей, Джарвис». До срабатывания модель Gemini не вызывается. После ответа сателлит возвращается к локальному ожиданию. Порог можно настроить, например `go run ./cmd/satellite -mode wake -wake-threshold 0.65`; снижение порога повышает чувствительность и число ложных срабатываний.
