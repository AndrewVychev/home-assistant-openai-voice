...
- локальный wake word «Куза» и post-wake очередь для каждого потока;
...
Gateway запускает отдельное состояние wake detector для каждого `satellite_id`. После «Кузы» wake-аудио отбрасывается, а все последующие PCM-фреймы на время подключения OpenAI/Gemini остаются в очереди. Клиент не содержит TensorFlow Lite и остаётся аппаратно независимым.
